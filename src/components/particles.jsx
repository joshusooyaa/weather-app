...
const ParticlesComponent = ( {type} ) => {
  const options = useMemo(() => {
    ...
    if (ClearSunny.includes(type)) {
      ind = 0;
    } else if (PartlyCloudy.includes(type)) {
      ind = 0;
    } else if (Cloudy.includes(type)) {
      ind = 0;
    } else if (LightRain.includes(type)) {
      ind = 1;
    } else if (RainHeavyRain.includes(type)) {
      ind = 2;
    } else if (Snowy.includes(type)) {
      ind = 3;
    }
